...
run = wandb.init(
    project="ppo_emulatednetwork_project",
    config=config,
...
class SpecialResetCallback(BaseCallback):
    def __init__(self, env, step_interval, verbose=0):
        super(SpecialResetCallback, self).__init__(verbose)
        self.env = env
        self.step_interval = step_interval

    def _on_step(self) -> bool:
        if self.num_timesteps % self.step_interval == 0:
            self.env.idsgame_env.resetflag = True
            if self.verbose > 0:
                print(f"Triggered special reset at step {self.num_timesteps}")
        return True
...
if __name__ == '__main__':
    ...
    special_reset_callback = SpecialResetCallback(env=env, step_interval=500, verbose=1)
    eval_env = env
    ...
    try:
        model.learn(
            ...
            callback=[WandbCallback(
                ...
                verbose=2,
            ), eval_callback, checkpoint_callback, EpisodeEndLoggingCallback(verbose=0), special_reset_callback]
        )
