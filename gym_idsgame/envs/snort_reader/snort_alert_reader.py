...
def check_pid():
    ...
    def write_pid(pid_name, pid_value):
        ...
        pid_file.write(str(pid_value).encode('utf-8'))
        pid_file.close()
    # check pid
    def pid_exists(pid):
        if is_linux:
...
class LogReader():
    ...
    def __init__(self, conf):
        ...
        self.pattern            = re.compile(r"^(\d+/\d+-\d+:\d+:\d+).+?\]\s+\[([\d:]+)\]\s+(.+?)\s+\[.*?\]\s+\[Priority:\s+(\d+)\]\s+\{(.+?)\}\s+(\d+\.\d+\.\d+\.\d+):?(\d+)?\s+->\s+(\d+\.\d+\.\d+\.\d+):?(\d+)?")
        ...
        self.h3counter_1_priority = 0
        self.h3counter_2_priority = 0
        self.h3counter_3_priority = 0
        self.h3counter_4_priority = 0
        self.fwcounter_1_priority = 0
        self.fwcounter_2_priority = 0
        self.fwcounter_3_priority = 0
        self.fwcounter_4_priority = 0
        self.wscounter_1_priority = 0
        self.wscounter_2_priority = 0
        self.wscounter_3_priority = 0
        self.wscounter_4_priority = 0
        self.macounter_1_priority = 0
        self.macounter_2_priority = 0
        self.macounter_3_priority = 0
        self.macounter_4_priority = 0
    ...
    def start_tail(self):
        ...
        while self.readflag == True:
            line = self.active_log_object.readline().strip()

            # pause collection when nothing to read
            if not line:
                self.variable_config.h3counter_1_priority = self.h3counter_1_priority
                self.variable_config.h3counter_2_priority = self.h3counter_1_priority
                self.variable_config.h3counter_3_priority = self.h3counter_1_priority
                self.variable_config.h3counter_4_priority = self.h3counter_1_priority
                self.variable_config.fwcounter_1_priority = self.fwcounter_1_priority
                self.variable_config.fwcounter_2_priority = self.fwcounter_2_priority
                self.variable_config.fwcounter_3_priority = self.fwcounter_3_priority
                self.variable_config.fwcounter_4_priority = self.fwcounter_4_priority
                self.variable_config.wscounter_1_priority = self.wscounter_1_priority
                self.variable_config.wscounter_2_priority = self.wscounter_2_priority
                self.variable_config.wscounter_3_priority = self.wscounter_3_priority
                self.variable_config.wscounter_4_priority = self.wscounter_4_priority
                self.variable_config.macounter_1_priority = self.macounter_1_priority
                self.variable_config.macounter_2_priority = self.macounter_2_priority
                self.variable_config.macounter_3_priority = self.macounter_3_priority
                self.variable_config.macounter_4_priority = self.macounter_4_priority
                self.variable_config.save()
                self.readflag = False
                self.save_state()
                print("Reading finished!, no more new log alerts found!")
            ...
            # save state file each 5 seconds
            if int(time.time()) % self.check_interval == 0:
                if not self.saved:
            ...
            #IDSfile='match_output.txt'
            #with open(IDSfile, 'a') as file:
                #for m in match:
                    #print("m is", m)
                    #file.write('; '.join(m) + '\n') 
                #print("Match data has been written to %s" % IDSfile) 
            ...
            if not match:
                ...
                snort_priority  = int(match[0][3])
                snort_proto     = match[0][4]
                snort_src_ip    = match[0][5]
                snort_src_port  = match[0][6]
                snort_dst_ip    = match[0][7]
                snort_dst_post  = match[0][8]
                
                #print("snort priortity is", snort_priority) 
                #print("destination ip is", snort_dst_ip)
                ...
                if snort_dst_ip == "100.0.0.40":
                    if snort_priority == 1:
                        self.wscounter_1_priority = self.wscounter_1_priority+1
                    if snort_priority == 2:
                        self.wscounter_2_priority = self.wscounter_2_priority+1
                    if snort_priority == 3:
                        self.wscounter_3_priority = self.wscounter_3_priority+1
                    if snort_priority == 4:
                        self.wscounter_4_priority = self.wscounter_4_priority+1
                
                if snort_dst_ip == "10.0.0.22" or snort_dst_ip == "100.0.0.1":
                    if snort_priority == 1:
                        self.fwcounter_1_priority = self.fwcounter_1_priority+1
                    if snort_priority == 2:
                        self.fwcounter_2_priority = self.fwcounter_2_priority+1
                    if snort_priority == 3:
                        self.fwcounter_3_priority = self.fwcounter_3_priority+1
                    if snort_priority == 4:
                        self.fwcounter_4_priority = self.fwcounter_4_priority+1
                    
                if snort_dst_ip == "10.0.1.2":
                    if snort_priority == 1:
                        self.h3counter_1_priority = self.h3counter_1_priority+1
                    if snort_priority == 2:
                        self.h3counter_2_priority = self.h3counter_2_priority+1
                    if snort_priority == 3:
                        self.h3counter_3_priority = self.h3counter_3_priority+1
                    if snort_priority == 4:
                        self.h3counter_4_priority = self.h3counter_4_priority+1   
                        
                if snort_dst_ip == "10.0.0.50":
                    if snort_priority == 1:
                        self.macounter_1_priority = self.macounter_1_priority+1
                    if snort_priority == 2:
                        self.macounter_2_priority = self.macounter_2_priority+1
                    if snort_priority == 3:
                        self.macounter_3_priority = self.macounter_3_priority+1
                    if snort_priority == 4:
                        self.macounter_4_priority = self.macounter_4_priority+1 
